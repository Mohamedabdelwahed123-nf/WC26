...
export function WebGLShader({ className }: { className?: string } = {}) {
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      // Se dimensionne à la taille réelle du canvas (conteneur), avec repli sur la fenêtre.
    ...
    // Suit aussi les changements de taille du conteneur (hero, etc.).
  ...
  return (
    <canvas
      ref={canvasRef}
      className={cn("block h-full w-full", className ?? "fixed left-0 top-0")}
    />
  );
}
